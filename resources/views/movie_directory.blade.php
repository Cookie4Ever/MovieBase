            ...
            <table class="table">
                @foreach($mostPopularMovies as $movie)
                    <tr>
                        ...
                        <td class="row-actors">
                            <a href="{{ route('getMovie', $movie['id']) }}">
                        ...
                        <td class="row-actors">
                            <a href="{{ route('getMovie', $movie['id']) }}">
                                <h2 id="character">{{ $movie['title'] }} ({{ strstr($movie['release_date'], '-', true) }})</h2>
                            </a>
            ...
            <table class="table">
                @foreach($topRatedMovies as $movie)
                    <tr>
                        ...
                        <td class="row-actors">
                            <a href="{{ route('getMovie', $movie['id']) }}">
                                <h2>{{ $movie['title'] }} ({{ strstr($movie['release_date'], '-', true) }})</h2>
                            </a>
                            <h4>{{ $movie['original_title'] }}</h4>
