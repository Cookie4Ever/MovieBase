...
@section('page-content')
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
            <h1>{{ $movie['title'] }} ({{ strstr($movie['release_date'], '-', true) }})</h1>
            <div style="text-align: justify">
            <?PHP
                if(strlen($movie['overview']) > 550)
                {
                    $movie['overview'] = substr($movie['overview'], 0, 550) . "...";
                    echo $movie['overview'];
                }
                else
                {
            ...
            <h4>Średnia ocena użytkowników portalu MovieBase: <span class="glyphicon glyphicon-star" aria-hidden="true"></span>
                @if(count($reviews) > 0){{ $ratingAverage }} @else Brak ocen @endif
            </h4>
            <h4>Średnia ocena użytkowników portalu Tmdb: <span class="glyphicon glyphicon-star" aria-hidden="true"></span>{{ $movie['vote_average'] }}</h4>

            <h3>Gatunki</h3>
                @foreach($movie['genres'] as $genre)
                    <a href="{{ route('movies_genre', $genre['id']) }}" class="btn btn-default genres"> {{ $genre['name'] }} </a>
                @endforeach
        </div>
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
            <br/>
            {{ Form::open(['route' => ['getMovie', $movie['id']]])  }}
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            ...
            @for($i = 0; $i < 4; $i++)
                <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3">
                    ...
                    <h4 id="character">{{ $credits['cast'][$i]['character'] }}</h4>
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <a href="{{ route('get_cast', $movie['id']) }}" class="btn btn-default btn-margin" >Pełna obsada i załoga</a>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            ...
            <a href="{{ route('get_review', $movie['id']) }}" class="btn btn-success btn-lg btn-block review-button">Napisz recenzje</a>
        </div>
    </div>
